...
namespace Shop.Infra.Data.Repositories
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<Order> GetUserBasket(long orderId, long userId)
        {
            ...
            if (orders != null)
                orders.OrderSum = await OrderSum(orderId);
        ...
        public async Task<Order> GetUserBasket(long userId)
        {
            ...
            var order = await _context.Orders
                 .Include(c => c.User)
                 .Include(c => c.OrderDetails)
                 .Where(c => c.UserId == userId && !c.IsDelete && !c.IsFinaly)
                 .Select(c => new Order
                 {
                     UserId = c.UserId,
                     CreateDate = c.CreateDate,
                     ...
                     IsFinaly = c.IsFinaly,
                     OrderSum = c.OrderSum,
                     OrderDetails = _context.OrderDetails.Where(c => !c.IsDelete && !c.Order.IsFinaly && c.Order.UserId == userId)
                     .Include(c => c.Product).ToList()
                 })
                 .FirstOrDefaultAsync();

            if(order != null)
            order.OrderSum = await OrderSum(order.Id);
